...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-amber-200 selection:text-slate-900 font-sans relative overflow-x-hidden">
      ...
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur-xl sticky top-0 z-40 px-4 sm:px-6 py-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          
          <div onClick={() => setActiveTab('home')} className="flex items-center gap-2 sm:gap-3 cursor-pointer group shrink-0">
          ...
          <nav className="hidden md:flex items-center gap-8 text-xs font-semibold tracking-wider uppercase">
            <button onClick={() => setActiveTab('home')} className={`transition-colors hover:text-amber-600 ${activeTab === 'home' ? 'text-amber-600' : 'text-slate-500'}`}>{t('nav_home', 'Главная')}</button>
            <button onClick={() => setActiveTab('services')} className={`transition-colors hover:text-amber-600 ${activeTab === 'services' ? 'text-amber-600' : 'text-slate-500'}`}>{t('nav_services', 'Услуги')}</button>
            <button onClick={() => setActiveTab('doctors')} className={`transition-colors hover:text-amber-600 ${activeTab === 'doctors' ? 'text-amber-600' : 'text-slate-500'}`}>{t('nav_doctor', 'Врач')}</button>
            <button onClick={() => setActiveTab('reviews')} className={`transition-colors hover:text-amber-600 ${activeTab === 'reviews' ? 'text-amber-600' : 'text-slate-500'}`}>{t('nav_reviews', 'Отзывы')}</button>
            <button onClick={() => setActiveTab('contacts')} className={`transition-colors hover:text-amber-600 ${activeTab === 'contacts' ? 'text-amber-600' : 'text-slate-500'}`}>{t('nav_contacts', 'Контакты')}</button>
          </nav>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="hidden lg:flex items-center gap-2 mr-1">
              <a href="https://instagram.com/" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-amber-500 transition-colors" title="Instagram">
              ...
              <a href="https://facebook.com/" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-amber-500 transition-colors" title="Facebook">
              ...
              <a href="https://tiktok.com/" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-amber-500 transition-colors" title="TikTok">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z"/></svg>
              </a>
            </div>

            <div className="flex gap-0.5 sm:gap-1 bg-slate-100 border border-slate-200 rounded-lg p-0.5 sm:p-1">
              {['ru', 'ua', 'de', 'en'].map((lang) => (
                <button 
                  key={lang}
                  onClick={() => i18n.changeLanguage(lang)} 
                  className={`text-[9px] sm:text-[10px] font-bold uppercase px-1.5 sm:px-2 py-1 rounded transition-all ${i18n.language === lang ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500 hover:text-slate-900'}`}
                >
                  {lang}
                </button>
              ))}
            ...
            <a href="tel:+380443345678" className="hidden lg:flex items-center gap-2 bg-slate-50 border border-slate-200 hover:border-amber-300 px-3.5 py-2 rounded-xl text-xs font-bold text-slate-700 transition-colors">
              <span>+38 (044) 334-56-78</span>
            </a>
            
            <button 
              onClick={() => setActiveTab('booking')}
              className="px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-wider bg-gradient-to-r from-amber-500 to-rose-500 text-white shadow-md shadow-amber-500/20 hover:shadow-amber-500/40 transition-all hover:-translate-y-0.5 shrink-0"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 w-full flex-grow relative z-10">
        ...
        {/* ОБНОВЛЕННАЯ СТРАНИЦА "УСЛУГИ" (СПИСКОМ) */}
        {activeTab === 'services' && (
        ...
        {/* ОБНОВЛЕННЫЙ БЛОК БРОНИРОВАНИЯ (MULTI-SELECT И БЛОКИРОВКА ВРЕМЕНИ) */}
        {activeTab === 'booking' && (
